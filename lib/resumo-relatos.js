...
// Descrição de danos (aba Danos) entra no resumo como um card próprio, um por
// veículo (chave `danosDe`); sem veículo escolhido, um card geral.
export function upsertDanosNoResumo(relatos, { texto, rotulo = '', chave = 'geral' }, novoId = () => Date.now()) {
  const lista = Array.isArray(relatos) ? relatos.map((r) => ({ ...r })) : [];
  const t = typeof texto === 'string' ? texto.trim() : '';
  if (!t) return lista;
  const entrada = { danosDe: String(chave), envolvidoNome: rotulo ? `Danos — ${rotulo}` : 'Danos materiais', texto: t };
  const idx = lista.findIndex((r) => r.danosDe === entrada.danosDe);
  if (idx >= 0) {
    lista[idx] = { ...lista[idx], ...entrada };
    return lista;
  }
  return [...lista, { id: novoId(), ...entrada }];
}

export function salvarDanosNoResumo(dados) {
  if (!dados?.texto?.trim()) return { ok: false, reason: 'vazio' };
  const atual = loadResumoState();
  const relatos = upsertDanosNoResumo(atual.relatos, dados);
  persistResumoState({ ...atual, relatos });
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('pmrv-resumo-changed'));
  }
  return { ok: true, total: relatos.length };
}
