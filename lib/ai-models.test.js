...
describe('modeloTemVisao', () => {
  it('reconhece famílias multimodais e recusa modelos só de texto', () => {
    ['mistral-small-latest', 'meta-llama/llama-4-scout-17b-16e-instruct', 'Qwen/Qwen2.5-VL-7B-Instruct', 'glm-4.6v-flash', 'pixtral-12b'].forEach((m) =>
      expect(modeloTemVisao(m)).toBe(true)
    );
    ['openai/gpt-oss-120b', 'meta-llama/llama-3.3-70b-instruct:free', 'deepseek-ai/DeepSeek-V3.1', 'Hermes-4-70B'].forEach((m) =>
      expect(modeloTemVisao(m)).toBe(false)
    );
    PMRV_MODELOS_VISAO.forEach((m) => {
      expect(aceitaImagem(obterProvedor(m.provedor), m.id)).toBe(true);
      expect(modeloValido(m.id)).toBe(true);
      expect(m.descricao.length).toBeGreaterThan(10);
    });
  });
});

describe('filtrarModelosVisao', () => {
  it('OpenRouter: só gratuitos que recebem imagem e respondem texto', () => {
    const bruto = {
      data: [
        { id: 'a/vl:free', name: 'A VL (free)', architecture: { input_modalities: ['text', 'image'], output_modalities: ['text'] } },
        { id: 'b/texto:free', architecture: { input_modalities: ['text'] } },
        { id: 'c/vl-pago', architecture: { input_modalities: ['image'] }, pricing: { prompt: '0.1', completion: '0.1' } },
        { id: 'openrouter/free', architecture: { input_modalities: ['image'] } },
        { id: 'd/gera-imagem:free', architecture: { input_modalities: ['image'], output_modalities: ['image'] } },
      ],
    };
    expect(filtrarModelosVisao('openrouter', bruto)).toEqual([
      expect.objectContaining({ id: 'a/vl:free', label: 'A VL', provedor: 'openrouter' }),
    ]);
    expect(filtrarModelosVisao('groq', bruto)).toEqual([]);
  });

  it('todos os modelos de foto fixos informam o plano gratuito', () => {
    PMRV_MODELOS_VISAO.forEach((m) => expect(m.gratis).toBeTruthy());
  });
});
