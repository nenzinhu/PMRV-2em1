...
export const PMRV_OPENROUTER_MODEL = 'nvidia/nemotron-3-ultra-550b-a55b:free';
// Modelo gratuito do Gemini (via endpoint compatível com OpenAI). "-latest"
// sempre aponta pro modelo flash mais atual do Google, evitando quebrar quando
// uma versão específica (ex.: gemini-2.0-flash) é descontinuada.
export const PMRV_GEMINI_MODEL = 'gemini-flash-latest';
...
export const PMRV_AI_PROVIDERS = [
  { id: 'groq', label: 'Groq', hint: 'Rápido, chave no servidor' },
  { id: 'openrouter', label: 'OpenRouter', hint: 'Vários modelos (ex.: GLM)' },
  { id: 'gemini', label: 'Gemini', hint: 'Google, modelo gratuito' },
];
...
export function obterProvedorIA() {
  ...
  return PMRV_PROVIDER_IDS.includes(salvo) ? salvo : 'groq';
}

export function definirProvedorIA(provedor) {
  const id = PMRV_PROVIDER_IDS.includes(provedor) ? provedor : 'groq';
...
// A chave real fica no servidor (.env.local / GROQ_API_KEY ou OPENROUTER_API_KEY)
...
export function obterChaveIA(forcarNova = false) {
  ...
  if (forcarNova) {
    const rotulo =
      provedor === 'openrouter' ? 'OpenRouter' : provedor === 'gemini' ? 'Gemini' : 'Groq';
    chave = window.prompt(
