...
import { obterModeloIA } from './ai-models';
...
export const PMRV_AI_PROVIDERS = [
  ...
  { id: 'openrouter', label: 'OpenRouter', hint: 'Vários modelos gratuitos (:free)' },];
...
export function obterChaveIA(forcarNova = false) {
  ...
  const chaveKey = provedor === 'openrouter' ? 'PMRV_OPENROUTER_KEY' : 'PMRV_GROQ_KEY';
  let chave = localStorage.getItem(chaveKey) || '';
  if (forcarNova) {
    const rotulo = provedor === 'openrouter' ? 'OpenRouter' : 'Groq';
    chave = window.prompt(
