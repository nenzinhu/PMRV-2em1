...
export const RODOVIAS = [
  'SC-401', 'SC-400', 'SC-402', 'SC-403', 'SC-404', 'SC-405', 'SC-406',
  'SC-281', 'SC-407', 'SC-408',
...
export function envolvidosText(list) {
  ...
  list.forEach((ev) => {
    ...
    const veic = PMRV_FOTO_SLOTS.filter((s) => s.veiculo && ev.fotos && ev.fotos[s.key]);
    const loc = PMRV_FOTO_SLOTS.filter((s) => !s.veiculo && ev.fotos && ev.fotos[s.key]);
    if (veic.length) {
      const vLabels = veic.map(
        (s) => `${s.label}${ev.fotoAngulos && ev.fotoAngulos[s.key] ? ` [${ev.fotoAngulos[s.key]}]` : ''}`
      );
      txt += `Fotos do veículo: ${vLabels.join(', ')}\n`;
    }
    if (loc.length) {
      txt += `Fotos do local: ${loc.map((s) => s.label).join(', ')}\n`;
    }
    if (!veic.length && !loc.length) txt += 'Fotos: nenhuma\n';
    txt += '\n';
  });
  return txt;
}
