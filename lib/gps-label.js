...
/** Extrai só cidade/UF do JSON do Nominatim, para preencher o campo Município. */
export function extrairCidadeUf(data) {
  if (!data || typeof data !== 'object') return { cidade: '', uf: '' };
  const a = data.address;
  if (!a || typeof a !== 'object') return { cidade: '', uf: '' };
  const cidade = a.city || a.town || a.village || a.municipality || '';
  const uf = ufFromState(a.state);
  return { cidade, uf };
}
  ...
  if (!info.foraDaRodovia && info.rodovia) {
    const km =
      typeof info.km === 'number'
        ...
        : String(info.km || '');
    return `${rodoviaLabel(info.rodovia) || info.rodovia} KM ${km}`.trim();
  }
