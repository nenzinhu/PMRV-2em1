import { PMRV_PROVEDORES_IA, modeloTemVisao, modeloValido, nomesEnv, obterProvedor, padraoVisao } from './ai-models';
...
// modeloTemVisao reconhecer, senão o padrão de visão de cada provedor.
export function montarTentativas({ provider, model, apiKeyCliente = '', fallback = true, visao = false }) {
  ...
  adicionar(escolhido, visao && !modeloTemVisao(pedido) ? padraoVisao(escolhido) : pedido, apiKeyCliente);
  if (fallback) {
