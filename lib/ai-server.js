import { PMRV_PROVEDORES_IA, aceitaImagem, limiteImagens, modeloValido, nomesEnv, obterProvedor, padraoVisao } from './ai-models';
...
// aceitaImagem permitir, senão o padrão de visão de cada provedor. Modelos
// com limite de fotos menor que `qtdImagens` são pulados.
export function montarTentativas({ provider, model, apiKeyCliente = '', fallback = true, visao = false, qtdImagens = 0 }) {
  const escolhido = obterProvedor(provider);
  const tentativas = [];

  const adicionar = (provedor, modelo, chaveExtra = '') => {
    if (!modelo) return;
    if (visao && qtdImagens > limiteImagens(provedor, modelo)) return;
  ...
  const padraoDe = (p) => (visao ? padraoVisao(p) : p.padrao);

  const pedido = modeloValido(model) ? model : escolhido.padrao;
  adicionar(escolhido, visao && !aceitaImagem(escolhido, pedido) ? padraoVisao(escolhido) : pedido, apiKeyCliente);
  if (fallback) {
